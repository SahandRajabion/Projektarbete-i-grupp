 ...
 class MessagesRepository extends Repository 
 {
	...
	public function GetMoreMessages($last_id, $userId) 
	{
 	...
 	public function GetMessagesForUser($userId) {	
		...
		{
			$array = array();
			$sql = "SELECT * FROM $this->table  WHERE ".self::$UserId."= ? ORDER BY " . self::$MsgId . " DESC LIMIT 0,6";
			$query = $this->db->prepare($sql);
			$params = array($userId);
			$query->execute($params);
	...
	public function getMsgByUserName($username) {
		...
		{
			$array = array();
			$sql = "SELECT * FROM sentmsg WHERE ".self::$FromName."= ? ORDER BY " . self::$MsgId . " DESC LIMIT 0,12";
			$query = $this->db->prepare($sql);
