...
require_once('Model/PostCourse.php');
	...
	else
	{
		if ($uploadPage->getTitle() != "") 
		{
			if (!preg_match($regExYoutube, $uploadPage->getTitle())) 
			{
				...
				$post = new PostCourse($uploadPage->getTitle(), $loginController->getId(),3);
				$id = $postModel->addCoursePost($post);
